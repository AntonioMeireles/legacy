...
time cvc cook --context $CONTEXT --build-label=$BRANCH group-desktop-platform=$BRANCH'[~!bootstrap,~!builddocs,~!dom0,~!domU,~!vmware,~!xen  is: x86 x86_64]' group-desktop-platform=$BRANCH'[~!bootstrap,~!builddocs,~!dom0,~!domU,~!vmware,~!xen is: x86]'  --allow-flavor-change  --debug=all 2>&1 |tee log
date
